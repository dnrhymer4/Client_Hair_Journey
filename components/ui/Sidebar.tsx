...
const ROLE_LABELS: Record<AppRole, string> = { mentor:"Mentor", mentee:"Mentee", client:"Client", admin:"Administrator" };

export function Sidebar({ view, setView, role }: { view: View; setView: (v: View) => void; role: AppRole }) {
  const { user, signOut } = useAuth();
  const nav = role === "client" ? NAV_CLIENT : NAV_MAIN;
    ...
    <aside style={{ width:210, background:"#0f0c0e", borderRight:"1px solid rgba(255,255,255,.06)", display:"flex", flexDirection:"column", flexShrink:0 }}>
      ...
      <nav style={{ flex:1, padding:"0 10px", display:"flex", flexDirection:"column", gap:2 }}>
        {nav.map(({ id, label, icon: Icon }) => (
          <button key={id} onClick={() => setView(id)} style={{ display:"flex", alignItems:"center", gap:9, padding:"9px 10px", borderRadius:9, border:"none", cursor:"pointer", fontSize:12.5, fontWeight:view===id?600:500, textAlign:"left", width:"100%", background:view===id?"linear-gradient(90deg,#c4687a,#c46870)":"transparent", color:view===id?"#fff":"#6e5a66", transition:"all .15s" }}
      ...
      <div style={{ margin:"0 10px 10px", borderTop:"1px solid rgba(255,255,255,.05)", paddingTop:10 }}>
        <div style={{ padding:"10px", border:"1px solid rgba(255,255,255,.06)", borderRadius:12, marginBottom:6 }}>
          <div style={{ display:"flex", alignItems:"center", gap:9 }}>
            ...
            <div style={{ minWidth:0, flex:1 }}>
              ...
              <div style={{ fontSize:10, color:"#6e5a66" }}>{ROLE_LABELS[role]}</div>
            </div>
          </div>
        </div>
        <button onClick={signOut} style={{ display:"flex", alignItems:"center", gap:8, width:"100%", padding:"8px 10px", background:"transparent", border:"none", borderRadius:8, cursor:"pointer", color:"#6e5a66", fontSize:12, fontWeight:500 }}
